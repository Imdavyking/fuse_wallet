...
class TransactionPayload {
  ...
  factory TransactionPayload.esdtNFTTransfert(
    String collection,
    int balance,
    int nonce,
    Address receiver, {
    ...
    final amount = balance.toRadixString(16);
    final nftNonce = nonce.toRadixString(16);
    final _arguments = [
      convert.hex.encode(utf8.encode(collection)),
      nftNonce.length % 2 == 0 ? nftNonce : '0$nftNonce',
      amount.length % 2 == 0 ? amount : '0$amount',
      convert.hex.encode(receiver.pubkey),
      if (methodName.isNotEmpty) convert.hex.encode(utf8.encode(methodName)),
      if (arguments.isNotEmpty)
        ...arguments
            .map((element) => convert.hex.encode(utf8.encode(element)))
    ...
    return _payloadFromCommandAndArguments('ESDTNFTTransfer',
        arguments: _arguments);
  }
