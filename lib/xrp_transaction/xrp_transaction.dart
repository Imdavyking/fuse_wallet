...
String encodeXrpJson(Map sampleXrpJson) {
  ...
  if (isXrp_X_Address(sampleXrpJson['Destination'])) {
    ...
    int? tag = destinationXDetails.tag;
    if (tag != null) sampleXrpJson['DestinationTag'] = tag;
    ...
    int? tag = sourceXDetails.tag;
    if (tag != null) sampleXrpJson['SourceTag'] = tag;
  }
  ...
  for (int i = 0; i < sorted.length; i++) {
    ...
    late Uint8List associatedValue;
    ...
    if (sortedKeys == 'TransactionType') {
      ...
      associatedValue = HEX.decode(sampleXrpJson[sortedKeys]) as Uint8List;
    }
...
Map signXrpTransaction(String privateKeyHex, Map xrpTransactionJson) {
  ...
  final signature = _encodeSignatureToDER(
    sign(
      firstsha512 as Uint8List,
      HEX.decode(privateKeyHex) as Uint8List,
    ),
  );
...
class XDecodedResult {
  ...
  XDecodedResult({
    ...
    required this.isTestNetwork,
    this.tag,
  });
}

XDecodedResult xaddress_to_classic_address(String X_Address) {
  ...
  return XDecodedResult(
    classicAddress: classic_address,
    isTestNetwork: isXTestNet,
    tag: tag,
  );
}
...
int? _get_tag_from_buffer(Uint8List buffer) {
  ...
  if (!BytesUtils.bytesEqual(List.filled(8, 0), buffer.sublist(1, 9))) {
