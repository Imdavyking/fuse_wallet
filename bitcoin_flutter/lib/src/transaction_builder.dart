...
class TransactionBuilder {
  ...
  TransactionBuilder({NetworkType? network, int? maximumFeeRate})
      : this.network = network ?? bitcoin,
        ...
        this._inputs = [],
        this._tx = Transaction()..version = 2;
  ...
  sign(
    ...
    for (var i = 0; i < input.pubkeys!.length; i++) {
      if (HEX
              .encode(ourPubKey as List<int>)
              .compareTo(HEX.encode(input.pubkeys![i] as List<int>)) !=
          0) continue;
  ...
  Transaction _build(bool allowIncomplete) {
    if (!allowIncomplete) {
      if (_tx.ins.length == 0) throw ArgumentError('Transaction has no inputs');
