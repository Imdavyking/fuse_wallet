...
EmptyRecord exoticLibrary(BitString bits, List<Cell> refs) {
...
ExoticMerkleProof exoticMerkleProof(BitString bits, List<Cell> refs) {
  final reader = BitReader(bits);
...
ExoticMerkleUpdate exoticMerkleUpdate(BitString bits, List<Cell> refs) {
  final reader = BitReader(bits);
...
ExoticCell resolveExotic(BitString bits, List<Cell> refs) {
  ...
  if (type == 1) {
    return _resolvePruned(bits, refs);
  } else if (type == 2) {
    return _resolveLibrary(bits, refs);
  } else if (type == 3) {
    return _resolveMerkleProof(bits, refs);
  } else if (type == 4) {
    return _resolveMerkleUpdate(bits, refs);
  } else {
    throw 'Invalid exotic cell type: $type';
  }
}
