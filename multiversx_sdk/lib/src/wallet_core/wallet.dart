...
class Wallet {
  ...
  // Factory method to create a Wallet instance
  factory Wallet(UserSecretKey privateKey) {
  ...
  Future<TransactionHash> sendEsdtNFT({
    required IProvider provider,
    ...
    final payload =
        TransactionPayload.esdtNFTTransfert(collection, amount, nonce, to);
    final transaction = Transaction.esdtTransfert(
      chainId: networkConfiguration.chainId,
      gasLimit: GasLimit.forNFTTransfert(
        data: payload,
        gasPerDataByte: networkConfiguration.gasPerDataByte,
        minGasLimit: networkConfiguration.minGasLimit.value,
      ),
