...
ForKMap<T> _forkMap<T>(Map<String, T> src) {
  if (src.isEmpty) {
  ...
  return ForKMap<T>(left: left, right: right);
}
...
Node<T> _buildNode<T>(Map<String, T> src) {
  ...
  ForKMap<T> forkedMaps = _forkMap(src);
  var left = forkedMaps.left;
  var right = forkedMaps.right;
...
void _writeNode<T>(
  ...
  if (src is NodeLeaf<T>) {
    serializer(src.value, to);
  } else if (src is NodeFork<T>) {
